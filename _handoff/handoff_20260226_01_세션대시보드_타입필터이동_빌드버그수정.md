---
title: 세션 대시보드 타입 필터 이동 + 빌드 버그 수정
created:
tags:
session_id: c9a111cb-b5af-430e-bae6-a253c6f3b77c
session_path:
plan:
---

# 세션 대시보드 타입 필터 이동 + 빌드 버그 수정
...
- `/ss-new` 임시 커맨드 생성 → 개발 레포 build.js를 직접 실행하는 경로로 우회
...
### `~/.claude/commands/ss-new.md` (신규)
- 개발 레포의 build.js를 직접 실행하는 임시 커맨드
- marketplace 동기화 전까지 `/ss-new`로 최신 버전 사용
...
- **나중**: EEXIST 해결 후 `/plugin update`로 marketplace 동기화 → `/ss-new` 삭제하고 `/ss`로 통합
