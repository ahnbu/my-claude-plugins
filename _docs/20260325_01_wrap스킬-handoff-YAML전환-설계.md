...
| 검증 항목 | 검증 방법 | 결과 | 비고 |
|-----------|-----------|------|------|
| template.md YAML 전환 | 파일 내용 확인, `---`로 시작하는지 | ⏳ 미실행 | |
| next-handoff.mjs 파일 생성 기능 | `node next-handoff.mjs "" "테스트"` 실행, 파일 존재 확인 | ⏳ 미실행 | |
| 플레이스홀더 치환 | 생성된 파일에 `__TITLE__`, `__DATE__`, `__NN__` 대신 실제 값이 들어있는지 | ⏳ 미실행 | |
| SKILL.md Step 2-2, 2-3 수정 | 파일 내용 확인 | ⏳ 미실행 | |
| /wrap 실행 시 YAML 형식 생성 | `/wrap` 호출 후 handoff 첫 줄 `---` 확인 | ⏳ 미실행 | |
| BQ 헤더 미생성 | 생성된 handoff에 `> 날짜:` 패턴 없는지 확인 | ⏳ 미실행 | |
...
1. template.md 헤더 → YAML frontmatter 전환 (BQ 제거, 본문 구조 유지)
2. next-handoff.mjs → template.md 복제 + 플레이스홀더 치환 후 파일 생성
3. SKILL.md Step 2-2, 2-3 → 스크립트가 파일 생성까지 담당한다는 지시로 업데이트
4. `/wrap` 실행하여 전체 검증
